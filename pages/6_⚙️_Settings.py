...
with st.container(border=True):
    ...
    with c1:
        ...
        task_reminders = st.toggle("Daily Task Digest", value=prefs.get("task_reminders", True))
    with c2:
        compact = st.toggle("Compact Mode", value=prefs.get("compact_mode", False))
    ...
    if st.button("Save Preferences", type="primary"):
        db.update_user_profile_fields(user.get('email', ''), {
            ...
            "preferences.compact_mode": compact
        })
...
<div class="ds-card" style="border:1px solid #ff4d4d; background:rgba(255,77,77,0.05);">
    ...
    <p style="font-size:14px; opacity:0.8;">Revoke your access to this workspace. This cannot be undone.</p>
</div>
""", unsafe_allow_html=True)
