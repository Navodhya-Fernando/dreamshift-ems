...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const tasks = await Task.collection
      .find(
        {
          $or: [{ projectId: projectObjectId }, { projectId }, { project_id: projectId }],
        },
    ...
    const titleByTaskId = new Map(tasks.map((task) => [String(task._id), String(task.title || 'Task')]));
    ...
    const users = await loadWorkspaceUsers(workspaceId, currentUserId);
    ...
    const normalized = commentsRaw.map((comment) => {
      ...
      const normalizedTaskId = comment.taskId ? String(comment.taskId) : comment.task_id ? String(comment.task_id) : undefined;
      ...
      return {
        ...
        mentionHandles,
        reactions,
        task: normalizedTaskId ? { id: normalizedTaskId, title: titleByTaskId.get(normalizedTaskId) || 'Task' } : null,
        author: {
    ...
    const threads = buildThread(normalized);
    ...
    return NextResponse.json(
      {
        ...
        data: {
          threads,
          tasks: tasks.map((task) => ({ _id: String(task._id), title: String(task.title || 'Task') })),
        },
      },
      { status: 200 }
    );
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const workspaceId = String(project.workspaceId || project.workspace_id || '');
    const allowed = await hasWorkspaceAccess(currentUserId, workspaceId);
    if (!allowed) return NextResponse.json({ success: false, error: 'Forbidden project access' }, { status: 403 });
    ...
    if (taskId) {
      ...
      if (!task) {
        return NextResponse.json({ success: false, error: 'Task not found in this project' }, { status: 404 });
      }
    }
    ...
    const created = await Comment.collection.insertOne({
      ...
      projectId: projectObjectId,
      project_id: projectId,
      taskId: taskId && mongoose.isValidObjectId(taskId) ? new mongoose.Types.ObjectId(taskId) : taskId || undefined,
    ...
    for (const mentionUser of mentionUsers) {
      ...
      void notifyUser({
        ...
        emailSubject: `Mentioned in project discussion`,
      });
    }
